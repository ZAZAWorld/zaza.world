<?php
class AdminReportCompanyVipController extends BaseController {
    function getIndex () {
		$begin = null;
		if (Input::has('begin'))
		...
		$items = Company::where(function($q){
				$q->where('is_vip', 1)->orWhere('is_restorant', 1);
			})->orderBy('id', 'desc');
		...
		$ar = array();
        $ar['title'] = 'Report of company top';
		...
		return View::make('admin.report.company_vip.index', $ar);
	}	
