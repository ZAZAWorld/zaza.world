...
<div class='ad_dialog_edit open shadow'>
	<span class="ad_dialog_edit__close">x</span>
	<div class='ad_dialog_edit__body'>
		...
		@if (!($advert->auto_brand_id > 0) || in_array('special_title', $cat_props))
			<div class='row ad_dialog_edit__row'>
				<div class='col-md-15 ad_dialog_edit__label'>{{ TransWord::getArabic('Title') }}</div>
				<div class='col-md-85'>
		...
		@if (!in_array('youtube_false', $cat_props))
			<div class='row ad_dialog_edit__row'>
				<div class='col-md-15 ad_dialog_edit__label'>{{ TransWord::getArabic('Youtube link') }}</div>
				<div class='col-md-85'>
					...
					<div class='row ad_dialog_edit_photo__list'>
						...
						@foreach ($photos as $p)
							<div class='ad_dialog_edit_photo__square'>
								...
								<input type='hidden' name='ar_img[]' value='{{ $p->file }}'>
		...
		@if (!in_array('about_false', $cat_props))
			<div class='row ad_dialog_edit__row'>
				<div class='col-md-15 ad_dialog_edit__label'>{{ TransWord::getArabic('About text') }}</div>
				<div class='col-md-85'>
					<textarea class="ad_dialog_edit__text" rows="4" name='note'>{{ $about->note }}</textarea>
				</div>
			</div>
		@endif
		@if (!in_array('price_false', $cat_props))
			<div class='row ad_dialog_edit__row'>
				<div class='col-md-15 ad_dialog_edit__label'>{{ TransWord::getArabic('Price') }}</div>
				<div class='col-md-40'>
					<input name="price" type="text" class="ad_dialog_edit__text" value='{{ $advert->price }}' />
				</div>
				<div class='col-md-45'>
					@if ($price_option['negotiable'])
						<input name="negotiable" type="checkbox" value="1" {{ ( $advert->negotiable ? 'checked': null) }}>{{ TransWord::getArabic('Negotiable') }} &nbsp;&nbsp;
					@endif
					@if ($price_option['exchange'])
						<input name="exchange" type="checkbox" value="1" {{ ( $advert->exchange ? 'checked': null) }}>{{ TransWord::getArabic('Exchange') }} &nbsp;&nbsp;
					@endif
					@if ($price_option['free'])
						<input name="free" type="checkbox" value="1" {{ ( $advert->free ? 'checked': null) }}>{{ TransWord::getArabic('Free') }}
					@endif
				</div>
			</div>
		@endif
		@if ($advert->auto_brand_id > 0 && $advert->auto_model_id > 0)
			<div class='row ad_dialog_edit__row'>
				<div class='col-md-15 ad_dialog_edit__label'>{{ TransWord::getArabic('Brand') }}</div>
				<div class='col-md-85'>
			...
			<div class='row ad_dialog_edit__row'>
				<div class='col-md-15 ad_dialog_edit__label'>{{ TransWord::getArabic('Model') }}</div>
				<div class='col-md-85'>
			...
			<div class='row ad_dialog_edit__row'>
				<div class='col-md-15 ad_dialog_edit__label'>{{ TransWord::getArabic('Year') }}</div>
				<div class='col-md-85'>
		...
		@foreach ($sys_props as $prop) 
			...
			@endif
			<div class='row ad_dialog_edit__row'>
				<div class='col-md-15 ad_dialog_edit__label'>{{ TransWord::getArabic($prop->name) }}</div>
				<div class='col-md-85'>
					@if ($prop->type_id == 4)
						<!------ radio button ------->
		...
		<div class='row ad_dialog_edit__row'>
			<div class='col-md-15 ad_dialog_edit__label'>{{ TransWord::getArabic('City') }}</div>
			<div class='col-md-85'>
				{{  Form::select('city_id', $ar_city, $advert->city_id, array('class'=>'ad_dialog_edit__text', 'required'=>'required')) }}
			</div>
		</div>
		<div class='row ad_dialog_edit__row'>
			<div class='col-md-15 ad_dialog_edit__label'>{{ TransWord::getArabic('Address') }}</div>
			<div class='col-md-85'>
	...
	<div class='ad_dialog_edit_bottom'>
		...
		<button class="ad_dialog_edit__submit" type="submit">{{ TransWord::getArabic('Send') }}</button>
	</div>
