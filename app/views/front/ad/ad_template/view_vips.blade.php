<div class="advert"  data-id='{{ $ad->id }}'>
	...
	<a href="#link-advert" class="advert__link" data-id='{{ $ad->id }}' data-ids='{{ implode(',', $vip_ads_ids) }}'>
        <?php
        $files_count = AdvertFile::where('advert_id', $ad->id)->count();
        ?>
		@if($files_count > 1)
				<div class="advert__image-block {{ ($ad->checkView() ? 'normal_blur' : null ) }}">
					<div class="advert__image-block_slides" style="background-image:url('{{ $ad->photo }}');">
					</div>
				</div>
