jQuery( document ).ready( function( $ ) {
	...
	*/
	...
	function createAdvertDialog (id,ids) {
		$.post("/ad/view", { id: id, ids:ids} ).done(function( data ) {
			if(data == '' || data == '0')
					return false;
	...
	function openAdvertDialog (advert) {
		...
		var ids = advert.data('ids');

		if (($(".ad-modal").length > 0)){ // has open ad dialog
			$('.ad-modal').remove();
			createAdvertDialog(id,ids);
	...
	$('.js_avdform_photo_upload').change(function(){
		var file = $(this);
        var formData = new FormData();
        formData.append( 'image', $(this)[0].files[0] );
        $.ajax({
            ...
            success: function (data) {
                if (data!='0' && data!='1') {
					$('.js_avdform_photo_preview').css('display', 'block');
					$('.js_avdform_photo_preview').attr('src', data);
                }
            },
            cache: false,
            contentType: false,
	...
	$('.js-avdform_count-input').change(function () {
		        var text = 'AED ';
		        ...
		        $('.c-avdform__advertising__currency').html(text);		      
	});
